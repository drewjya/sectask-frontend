...
reportFileDialog.onChange((files) =>
  upload({
    files,
    type: "report",
  })
);
attachmentFileDialog.onChange((files) =>
  upload({
    files,
    type: "attachment",
  })
...
const canUpload = computed(() => {
  return props.maxDate.valueOf() > Date.now();
});
</script>
...
<template>
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
    <VFileList
      title="Reports"
      :files="props.reports"
      :canUpload="myRole === Role.TECHNICAL_WRITER && canUpload"
      @upload="
        () => {
    ...
    <VFileList
      ...
      :canUpload="myRole === Role.DEVELOPER && canUpload"
      @upload="
        () => {
