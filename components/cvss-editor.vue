...
const app = useApp();
const newC = computed(() =>
  app.sidebar ? "w-[calc(100vw-40rem)]" : "w-[calc(100vw-22rem)]"
);
</script>
...
<template>
  <div class="flex flex-col gap-4" v-if="store.cvss">
      ...
      <div class="cvsscontainer">
        ...
        <div class="cvssscroll">
          <CvssValue
            ...
            :items="[network, adjacent, low, physical]"
            :selected="store.cvss?.AV"
            @select="
              (val) => {
                if (!store.cvss) {
                  return;
      ...
      <div class="cvsscontainer">
        <div class="cvssscroll">
          <CvssValue
            ...
            "
            :selected="store.cvss.S"
            :items="[notdefined, negligible, present]"
          />
          <CvssValue
            ...
            "
            :selected="store.cvss.AU"
            :items="[notdefined, no, yes]"
          />
          <CvssValue
            label="Recovery (R)"
            @select="
            ...
            "
            :selected="store.cvss.R"
            :items="[notdefined, automatic, user, irrecovable]"
          />
          <CvssValue
            ...
            "
            :selected="store.cvss.V"
            :items="[notdefined, difuse, concentrated]"
          />
          <CvssValue
            ...
            "
            :selected="store.cvss.RE"
            :items="[notdefined, low, moderate, high]"
          />
          <CvssValue
            ...
            "
            :selected="store.cvss.U"
            :items="[notdefined, clear, green, amber, red]"
          />
        </div>
      </div>
    </div>
    <div class="flex flex-col gap-2" :class="newC">
      <CvssTitle
        title="Environmental (Modified Base Metrics)"
        type="consumer"
      />
      <div class="cvsscontainer">
        <div class="subtitle">Expoloitability Metrics</div>
        <div class="cvssscroll">
          <CvssValue
            ...
            "
            :selected="store.cvss.MAV"
            :items="[notdefined, network, adjacent, local, physical]"
          />
          <CvssValue
            ...
            "
            :selected="store.cvss.MAC"
            :items="[notdefined, low, high]"
          />
          <CvssValue
            ...
            "
            :selected="store.cvss.MAT"
            :items="[notdefined, none, present]"
          />
          <CvssValue
            label="Privileges Requirements (MPR)"
            @select="
              (val) => {
                if (!store.cvss) {
                  return;
            ...
            "
            :selected="store.cvss.MPR"
            :items="[notdefined, none, low, high]"
          />
          <CvssValue
            ...
            "
            :selected="store.cvss.MUI"
            :items="[notdefined, none, passive, active]"
          />
        </div>
      </div>
      <div class="cvsscontainer">
      ...
      <div class="cvsscontainer">
        ...
        <div class="cvssscroll">
          <CvssValue
            ...
            "
            :selected="store.cvss.MSC"
            :items="[notdefined, empty, high, low, negligible]"
          />
          <CvssValue
            label="Integrity (MSI)"
            @select="
            ...
            "
            :selected="store.cvss.MSI"
            :items="[notdefined, safety, high, low, negligible]"
          />
          <CvssValue
    ...
    <div class="flex flex-col gap-2" :class="newC">
      <CvssTitle
        title="Environmental (Security Requirements)"
        type="consumer"
      />
      <div class="cvsscontainer">
        <div class="cvssscroll">
          <CvssValue
            label="Confidentiality (CR)"
            @select="
              (val) => {
                if (!store.cvss) {
                  return;
          ...
          <CvssValue
            ...
            "
            :selected="store.cvss.IR"
            :items="ncia"
          />
          <CvssValue
            label="Availability (AR)"
            @select="
              (val) => {
                if (!store.cvss) {
                  return;
...
<style scoped>
.cvsscontainer {
  @apply w-full bg-gray-100 rounded-md p-2 dark:bg-gray-800;
}
