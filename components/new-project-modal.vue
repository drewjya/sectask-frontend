...
const search = async (query: string) => {
  ...
  return datas.filter(i => !formD.state.members.find(j => j.email === i.email))
        ...
        <section class="flex w-full flex-col gap-4">
          ...
          <vform class="flex w-full flex-col" :form="formD">
            ...
            <div class="mt-3 h-56 max-h-56 w-full flex-col overflow-y-auto rounded-md border border-gray-200">
              ...
              <template v-else>
                <div v-for="(invite, index) in formD.state.members" :key="invite.email"
          ...
          <UButton label="Submit" type="submit" block variant="solid" class="mt-2" @click="formD.submit.value" />
        </section>
      </template>
    </UCard>
