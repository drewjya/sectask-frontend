...
<template>
  <div class="flex flex-col h-full pb-4">
    <Header
      :image="store.picture ? `${fileUrl}${store.picture.name}` : undefined"
      :loading="store.loading"
      :project-manager="store.pm?.name ?? '-'"
      :id="store.id ?? -1"
      v-model:name="store.name!"
      v-model:range="store.range!"
      :my-role="store.myrole ? roleLabel(store.myrole) : '-'"
      :userCanUpdateHeader="store.myrole === Role.PM"
      type="project"
      :error="store.error"
    />
      ...
      <div class="flex flex-col h-full">
        ...
        <div class="grow h-full overflow-auto">
          <template v-if="store.loading">
            ...
            >
              <USkeleton
                class="size-full flex flex-col items-center justify-center opacity-70"
              />
            </div>
          </template>
          <template v-else>
            <section class="size-full flex-grow">
              <template
                v-if="
                  !route.query.tab ||
                  `${$route.query.tab}`.startsWith('subproject')
                "
              >
                <SubprojectTab />
              </template>
              <template v-else-if="`${$route.query.tab}`.startsWith('members')">
                <MembersTab
                  ...
                  :canInvite="store.myrole === Role.PM"
                  :isSubproject="false"
                  :doc-id="store.id ?? -1"
                  :loading="store.loading"
                />
              ...
              <template
                v-else-if="`${$route.query.tab}`.startsWith('calendar')"
              >
