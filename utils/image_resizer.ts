    ...
    return ({ node, editor, getPos }) => {
      ...
      const paintPositionContoller = () => {
        ...
        $postionController.setAttribute(
          "style",
          "position: absolute; top: 0%; left: 50%; width: 140px; height: 25px; z-index: 999; background-color: rgba(255, 255, 255, 0.7); border-radius: 4px; border: 2px solid #6C6C6C; cursor: pointer; transform: translate(-50%, -50%); display: flex; justify-content: space-between; align-items: center; padding: 0 10px;"
        );
        ...
        $deleteController.setAttribute(
          "src",
          "https://fonts.gstatic.com/s/i/short-term/release/materialsymbolsoutlined/close/default/20px.svg"
        );
        ...
        $deleteController.addEventListener("click", () => {
          ...
          $deleteController.remove();
          $centerController.remove();
          $container.remove();
          dispatchNodeView();
        });
