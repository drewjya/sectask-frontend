...
export async function imageResize(
  ...
  if (originalFile.size / 1000 < targetFileSizeKb) return originalFile; // File is already smaller
  ...
  while (Math.abs(file.size / 1000 - targetFileSizeKb) > maxDeviation) {
    ...
    file = await new Promise((resolve) =>
      canvas.toBlob((data) => resolve(data as Blob))
    );

    if (file.size / 1000 < targetFileSizeKb - maxDeviation) low = middle;
    else if (file.size / 1000 > targetFileSizeKb) high = middle;
...
export const fileUrl = (file?: VFile) => {
